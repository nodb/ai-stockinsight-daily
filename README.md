...
- 설정 가능한 Top N 심층 분석과 나머지 기사 Quick View 생성
- Jinja2 HTML 템플릿 기반 SMTP 발송
- GitHub Actions 스케줄 실행

## 리포트 구성

- `오늘의 핵심 키워드`: 전체 분석 결과에서 많이 등장하거나 중요도가 높은 키워드 5개를 보여줍니다.
- `Top N Deep Dive`: 랭킹 상위 N개 기사의 핵심 요약, 시장 영향 라벨, 투자 관점, 핵심 키워드, 수혜 섹터, 리스크를 보여줍니다.
- `Quick View`: 심층 분석 대상 이후의 주요 기사를 대표 섹터별로 묶어 보여줍니다.

Deep Dive의 `주의 필요` 라벨은 Gemini가 해당 기사에 중요한 불확실성이나 하방 리스크가 있다고 판단한 경우에만 표시됩니다.
...
| `DEEP_DIVE_COUNT` | 심층 분석 기사 수. 기본값 `10` |
| `MAIL_USER` | 발신 이메일 |
| `MAIL_PWD` | SMTP 앱 비밀번호 |
| `MAIL_TO` | 수신 이메일. 쉼표로 다중 지정 가능. 예: `a@example.com,b@example.com` |
...
여러 명에게 동시에 보내려면 `MAIL_TO` 값을 쉼표로 구분해 등록합니다.

```text
MAIL_TO=nodb@sbisb.co.kr,user2@example.com,user3@example.com
```
...
- `DEEP_DIVE_COUNT`: 심층 분석 기사 수입니다. Top 10으로 운영하려면 `10`을 등록하세요.
